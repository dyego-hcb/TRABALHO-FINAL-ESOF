<?php
    ...
    $email_alterar = $_POST['fieldEmailAlterar'];
    $telefone_alterar = $_POST['fieldTellAlterar'];

    if(isset($_POST['update']))
    {
        $sql = "SELECT * FROM usuarios WHERE email = '$email_alterar' AND idusuarios != '$iduser'";
        $result = $conexao->query($sql);

        if(mysqli_fetch_assoc($result)>0)
        {
            echo '<script>alert("Email ja cadastrado!!")</script>';
            header('Location: editarDadosContatoCliente.php');
        }
        else
        {
            $sqlUpdate = "UPDATE usuarios SET email='$email_alterar', telefone='$telefone_alterar' WHERE idusuarios = '$iduser'";
            $result = $conexao->query($sqlUpdate);
            echo '<script>alert("Alteracao de Dados feita com sucesso!!")</script>';
            header('Location: deslogarConta.php');
        }
    }
?>
<html lang="pt-br">
            ...
            <nav class="navbar navbar-expand-lg navbar-dark bg-dark border-bottom shadow-sm mb-3">
                ...
                <main class="flex-fill">
                    <div class="container">
                        ...
                        <hr>
                        <form action="editarDadosContatoCliente.php" method="POST" class="mt-3">
                            <div class="row d-flex justify-content-center">
                                <div class="col-sm-12 col-md-6">
                                    <fieldset class="row gx-3">
                                        <legend>Dados de Contato</legend>
                                        <div class="form-floating mb-3 col-md-8">
                                            <input class="form-control" type="email" id="fieldEmailAlterar" name="fieldEmailAlterar" placeholder=" " required/>
                                            <label for="txtEmail">E-mail</label>
                                        </div>
                                        <div class="form-floating mb-3 col-md-6">
                                            <input class="form-control" placeholder=" " type="text" name="fieldTellAlterar" id="fieldTellAlterar" />
